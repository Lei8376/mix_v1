...
class CaptionODISE(ODISE):
    ...
    def forward(self, batched_inputs):
        ...
        #print(outputs)
        outputs["images"] = denormalized_images.tensor
        #print("outputs:", outputs["mask_embed"].shape)
        ...
        if self.training:
            ...
            if "pred_open_logits" not in outputs:
                ...
                outputs["pred_open_logits"], mask_embed = self.cal_pred_open_logits(outputs)
            ...
            for mask_cls_result, mask_pred_result, input_per_image, image_size in zip(
                ...
                if self.panoptic_on:
                    ...
                    cur_masks = mask_pred_result[keep]
                    #print(cur_masks.shape)
                    cur_masks = cur_masks[:,:-1]
                    #print(cur_masks.shape)
                    print("cur_scores.view(-1,1,1).shape:", cur_scores.view(-1,1,1).shape)
                    cur_prob_masks = cur_scores.view(-1, 1, 1) * cur_masks
                    print(cur_prob_masks)
                    print("cur_prob_masks.shape:",  cur_prob_masks.shape)

                    cur_mask_ids = cur_prob_masks.argmax(0)
                    print(cur_mask_ids.shape)
                    #print("cur_mask_ids:", cur_mask_ids)
                    for k in range(cur_classes.shape[0]):
                        ...
                        #print(mask)
                        #print(mask.shape)
                        #here we have the mask, mask_embedding, class name
                """
                # instance segmentation inference
                if self.instance_on:
                    print("running retry_if_cuda_oom")
                    instance_r = retry_if_cuda_oom(self.instance_inference)(
                        mask_cls_result, mask_pred_result
                    )
                    processed_results[-1]["instances"] = instance_r

            #print("mask_cls_result:", mask_cls_result)
            #print("mask_pred_result:", mask_pred_result)
            return processed_results, mask_embed
